...
the code doesn't work because it tries filter on the year column after that column has been removed by select() so the year no longer exists when filtering applied. 
...
```{r}
HarryPotter <- movies %>% 
  filter(str_detect(title, "HarryPotter"))
...
library(dplyr)
movies %>%
filter((Animation == 1| Comedy == 1),votes >=100)%>% arrange(desc(budget)) %>% slice(1)  
```

5. Is your favorite movie in the data set? What other questions could you ask or answer using `filter()`, `select()`, and `arrange()`?
